...
DWORD TrainerBase::getID()        //得到窗口进程句柄
{
	...
	if (GetProcessIDFromName(GameCaptionSpawn, &id))//使用进程名获取ID
	{
		//3 取窗口进程句柄及权限
		curCap = GameCaptionSpawn;
		pid = OpenProcess(PROCESS_ALL_ACCESS, false, id);
		return id;
	}
	if (GetProcessIDFromName(GameCaptionAres, &id))//使用进程名获取ID
	{
		//3 取窗口进程句柄及权限
		curCap = GameCaptionAres;
		pid = OpenProcess(PROCESS_ALL_ACCESS, false, id);
		return id;
	}
	return 0;	//获取失败
